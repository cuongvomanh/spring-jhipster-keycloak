...
class MykprojectKafkaResourceIT {
    ...
    void setup() {
        ...
        MykprojectKafkaResource kafkaResource = new MykprojectKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/mykproject-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/mykproject-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
